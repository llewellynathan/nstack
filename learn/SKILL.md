...
version: 1.0.0
description: |
  Manage project learnings. Review, search, prune, and export what nstack
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"learn","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.nstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/nstack/bin/nstack-timeline-log '{"skill":"learn","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
see what nstack has learned across sessions on this project, search for relevant
...
```bash
eval "$(~/.claude/skills/nstack/bin/nstack-slug 2>/dev/null)"
~/.claude/skills/nstack/bin/nstack-learnings-search --limit 20 2>/dev/null || echo "No learnings yet."
```
...
nstack will automatically capture patterns, pitfalls, and insights it discovers."
...
```bash
eval "$(~/.claude/skills/nstack/bin/nstack-slug 2>/dev/null)"
~/.claude/skills/nstack/bin/nstack-learnings-search --query "USER_QUERY" --limit 20 2>/dev/null || echo "No matches."
```
...
```bash
eval "$(~/.claude/skills/nstack/bin/nstack-slug 2>/dev/null)"
~/.claude/skills/nstack/bin/nstack-learnings-search --limit 100 2>/dev/null
```
...
```bash
eval "$(~/.claude/skills/nstack/bin/nstack-slug 2>/dev/null)"
~/.claude/skills/nstack/bin/nstack-learnings-search --limit 50 2>/dev/null
```
...
LEARN_FILE="$NSTACK_HOME/projects/$SLUG/learnings.jsonl"
...
```bash
~/.claude/skills/nstack/bin/nstack-learnings-log '{"skill":"learn","type":"TYPE","key":"KEY","insight":"INSIGHT","confidence":N,"source":"user-stated","files":["FILE1"]}'
```
