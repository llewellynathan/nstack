...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"benchmark","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.nstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/nstack/bin/nstack-timeline-log '{"skill":"benchmark","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
mkdir -p .nstack/benchmark-reports
mkdir -p .nstack/benchmark-reports/baselines
```
...
Write to `.nstack/benchmark-reports/baselines/baseline.json`.
...
Write to `.nstack/benchmark-reports/{date}-benchmark.md` and `.nstack/benchmark-reports/{date}-benchmark.json`.
