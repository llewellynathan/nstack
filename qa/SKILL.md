...
description: |
  ...
  fix evidence, and a ship-readiness summary. For report-only mode, use /qa-only. (nstack)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.nstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/nstack/bin/nstack-timeline-log '{"skill":"qa","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
| Mode | full | `--regression .nstack/qa-reports/baseline.json` |
...
```bash
mkdir -p .nstack/qa-reports/screenshots
```
...
1. **Project-scoped test plans:** Check `~/.nstack/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   setopt +o nomatch 2>/dev/null || true  # zsh compat
   eval "$(~/.claude/skills/nstack/bin/nstack-slug 2>/dev/null)"
...
- Include full attribution comment:
  ...
  // Report: .nstack/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.nstack/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`
...
```bash
~/.claude/skills/nstack/bin/nstack-learnings-log '{"skill":"qa","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
