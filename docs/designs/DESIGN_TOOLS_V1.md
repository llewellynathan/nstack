# Design: nstack Visual Design Generation (`design` binary)
...
**Screenshot consent (first-time only for $D evolve):** "This will send a screenshot of your live site to OpenAI for design evolution. [Proceed] [Don't ask again]" Stored in ~/.nstack/config.yaml as design_screenshot_consent.
...
- Exploration variants go to `/tmp/nstack-mockups-{session}/` (ephemeral, not committed)
...
- Fallback: if not in a git repo, save to `/tmp/nstack-mockup-{timestamp}.png`
...
```typescript
// claude host:
designDir: '~/.claude/skills/nstack/design/dist'
// codex host:
designDir: '$NSTACK_DESIGN'
```
Note: Codex runtime setup (`setup` script) must also export `NSTACK_DESIGN` env var, similar to how `NSTACK_BROWSE` is set.
...
| `scripts/resolvers/preamble.ts` | Add `NSTACK_DESIGN` env var export for Codex host |
...
1. Read `~/.nstack/openai.json` → `{ "api_key": "sk-..." }` (file permissions 0600)
...
3. If neither exists → guided setup flow:
   ...
   - Write to `~/.nstack/openai.json` with 0600 permissions
...
- Symlinked via existing `~/.claude/skills/nstack/` install path
...
- Auth module (read ~/.nstack/openai.json, fallback to env var, guided setup flow)
...
- Add NSTACK_DESIGN env var export to `scripts/resolvers/preamble.ts` (Codex host)
