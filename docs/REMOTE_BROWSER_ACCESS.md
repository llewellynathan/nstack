# Remote Browser Access — How to Pair With a NStack Browser

A NStack Browser server can be shared with any AI agent that can make HTTP requests.
...
─────────────                         ────────────
NStack Browser Server                 Any AI agent
...
```bash
$B pair-agent --local openclaw    # writes to ~/.openclaw/skills/nstack/browse-remote.json
$B pair-agent --local codex       # writes to ~/.codex/skills/nstack/browse-remote.json
$B pair-agent --local cursor      # writes to ~/.cursor/skills/nstack/browse-remote.json
```
...
3. Save it: `echo 'NGROK_AUTHTOKEN=your_token' > ~/.nstack/ngrok.env`
4. Optionally claim a stable domain: `echo 'NGROK_DOMAIN=your-name.ngrok-free.dev' >> ~/.nstack/ngrok.env`
