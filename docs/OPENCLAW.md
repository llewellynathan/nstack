# nstack x OpenClaw Integration

nstack integrates with OpenClaw as a methodology source, not a ported codebase.
OpenClaw's ACP runtime spawns Claude Code sessions natively. nstack provides the
...
```
  ...
  calendar, memory, EA                     methodology + planning
       ...
       │   investigate, retro                   │
       │                                        ├── Generates nstack-lite
       ├── sessions_spawn(runtime: "acp")       │   (planning discipline)
       │       │                                │
       │       └── Claude Code                  ├── Generates nstack-full
       │           └── nstack installed at      │   (complete pipeline)
       │               ~/.claude/skills/nstack  │
...
OpenClaw decides at spawn time which tier of nstack support to use:

| Tier | When | Prompt prefix |
|------|------|---------------|
| **Simple** | One-file edits, typos, config changes | No nstack context injected |
| **Medium** | Multi-file features, refactors | nstack-lite CLAUDE.md appended |
| **Heavy** | Specific nstack skill needed | "Load nstack. Run /X" |
| **Full** | Complete features, objectives, projects | nstack-full pipeline appended |
| **Plan** | "Help me plan a Claude Code project" | nstack-plan pipeline appended |
...
The complete ready-to-paste section lives in `openclaw/agents-nstack-section.md`.
Copy it into your OpenClaw AGENTS.md.

Key behavioral rules (these go ABOVE the dispatch tiers):

1. **Always spawn, never redirect.** When the user asks to use ANY nstack skill,
...
nstack-lite/full as a new section. Do not replace the repo's existing instructions.

## What nstack generates for OpenClaw

All artifacts live in the `openclaw/` directory and are generated by
`bun run gen:skill-docs --host openclaw`:

### nstack-lite (Medium tier)
`openclaw/nstack-lite-CLAUDE.md` — ~15 lines of planning discipline:
...
### nstack-full (Full tier)
`openclaw/nstack-full-CLAUDE.md` — chains existing nstack skills:
...
### nstack-plan (Plan tier)
`openclaw/nstack-plan-CLAUDE.md` — full review gauntlet, no implementation:
...
- `nstack-openclaw-office-hours` — Product interrogation (6 forcing questions)
- `nstack-openclaw-ceo-review` — Strategic challenge (10-section review, 4 modes)
- `nstack-openclaw-investigate` — Operational debugging (4-phase methodology)
- `nstack-openclaw-retro` — Operational retrospective (weekly review)

Source lives in `openclaw/skills/` in the nstack repo. These are hand-crafted
adaptations of the nstack methodology for OpenClaw's conversational context.
No nstack infrastructure (no browse, no telemetry, no preamble).

## Spawned session detection

When Claude Code runs inside a session spawned by OpenClaw, the `OPENCLAW_SESSION`
environment variable should be set. nstack detects this and adjusts:
...
For OpenClaw users: tell your OpenClaw agent "install nstack for openclaw."

The agent should:
1. Install nstack-lite CLAUDE.md into its coding session templates
2. Install the 4 native methodology skills
3. Add dispatch routing to AGENTS.md
4. Verify with a test spawn

For nstack developers: `./setup --host openclaw` outputs this documentation.
...
- No SOUL.md from nstack (OpenClaw has its own)
