...
description: |
  ...
  Proactively suggest when user has approved a design or has a plan ready. (nstack)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-html","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.nstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/nstack/bin/nstack-timeline-log '{"skill":"design-html","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
_PRETEXT_VENDOR=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.claude/skills/nstack/design-html/vendor/pretext.js" ] && _PRETEXT_VENDOR="$_ROOT/.claude/skills/nstack/design-html/vendor/pretext.js"
[ -z "$_PRETEXT_VENDOR" ] && [ -f ~/.claude/skills/nstack/design-html/vendor/pretext.js ] && _PRETEXT_VENDOR=~/.claude/skills/nstack/design-html/vendor/pretext.js
...
$B screenshot /tmp/nstack-verify-mobile.png --width 375
$B screenshot /tmp/nstack-verify-tablet.png --width 768
$B screenshot /tmp/nstack-verify-desktop.png --width 1440
```
