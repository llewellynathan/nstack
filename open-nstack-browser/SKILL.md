---
name: open-nstack-browser
version: 0.2.0
description: |
  Launch NStack Browser — AI-controlled Chromium with the sidebar extension baked in.
  Opens a visible browser window where you can watch every action in real time.
  The sidebar shows a live activity feed and chat. Anti-bot stealth built in.
  Use when asked to "open nstack browser", "launch browser", "connect chrome",
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"open-nstack-browser","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.nstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/nstack/bin/nstack-timeline-log '{"skill":"open-nstack-browser","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
# /open-nstack-browser — Launch NStack Browser

Launch NStack Browser — AI-controlled Chromium with the sidebar extension,
...
if [ -f "$(git rev-parse --show-toplevel 2>/dev/null)/.nstack/browse.json" ]; then
  _OLD_PID=$(cat "$(git rev-parse --show-toplevel)/.nstack/browse.json" 2>/dev/null | grep -o '"pid":[0-9]*' | grep -o '[0-9]*')
  [ -n "$_OLD_PID" ] && kill "$_OLD_PID" 2>/dev/null || true
  sleep 1
  [ -n "$_OLD_PID" ] && kill -9 "$_OLD_PID" 2>/dev/null || true
  rm -f "$(git rev-parse --show-toplevel)/.nstack/browse.json"
fi
# Clean Chromium profile locks (can persist after crashes)
_PROFILE_DIR="$HOME/.nstack/chromium-profile"
...
This launches NStack Browser (rebranded Chromium) in headed mode with:
- A visible window you can watch (not your regular Chrome — it stays untouched)
- The nstack sidebar extension auto-loaded via `launchPersistentContext`
- Anti-bot stealth patches (sites like Google and NYTimes work without captchas)
- Custom user agent and NStack Browser branding in Dock/menu bar
- A sidebar agent process for chat commands

The `connect` command auto-discovers the extension from the nstack install
...
```bash
cat "$(git rev-parse --show-toplevel 2>/dev/null)/.nstack/browse.json" 2>/dev/null | grep -o '"port":[0-9]*' | grep -o '[0-9]*'
```
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.claude/skills/nstack/extension/manifest.json" ] && _EXT_PATH="$_ROOT/.claude/skills/nstack/extension"
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.claude/skills/nstack/extension/manifest.json" ] && _EXT_PATH="$HOME/.claude/skills/nstack/extension"
...
> Chrome is launched with nstack control. You should see Playwright's Chromium
...
>    already show the nstack icon if the extension loaded successfully
> 2. Click the **puzzle piece** → find **nstack browse** → click the **pin icon**
> 3. Click the pinned **nstack icon** in the toolbar
...
> 2. Look for **"nstack browse"** — it should be listed and enabled
...
>
> If the Side Panel badge stays gray (disconnected), click the nstack icon
...
> - Run any nstack skill (`/qa`, `/design-review`, `/benchmark`) and watch
