#!/usr/bin/env bun
/**
 * analytics — CLI for viewing nstack skill usage statistics.
 *
 * Reads ~/.nstack/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.nstack', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('nstack skill usage analytics');
