 ...
 *
 * Telemetry data flow:
 *   1. Always: local JSONL append to ~/.nstack/analytics/ (inline, inspectable)
 *   2. If _TEL != "off" AND binary exists: nstack-telemetry-log for remote reporting
 */
...
NSTACK_ROOT="$HOME/${hostConfig.globalRoot}"
[ -n "$_ROOT" ] && [ -d "$_ROOT/${ctx.paths.localSkillRoot}" ] && NSTACK_ROOT="$_ROOT/${ctx.paths.localSkillRoot}"
...
\`\`\`bash
${runtimeRoot}_UPD=$(${ctx.paths.binDir}/nstack-update-check 2>/dev/null || ${ctx.paths.localSkillRoot}/bin/nstack-update-check 2>/dev/null || true)
...
_PROACTIVE=$(${ctx.paths.binDir}/nstack-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f ~/.nstack/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$(${ctx.paths.binDir}/nstack-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <(${ctx.paths.binDir}/nstack-repo-mode 2>/dev/null) || true
REPO_MODE=\${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_LAKE_SEEN=$([ -f ~/.nstack/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$(${ctx.paths.binDir}/nstack-config get telemetry 2>/dev/null || true)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"${ctx.skillName}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.nstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
# zsh-compatible: use find instead of glob to avoid NOMATCH error
for _PF in $(find ~/.nstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "${ctx.paths.binDir}/nstack-telemetry-log" ]; then
      ${ctx.paths.binDir}/nstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
...
# Learnings count
eval "$(${ctx.paths.binDir}/nstack-slug 2>/dev/null)" 2>/dev/null || true
_LEARN_FILE="\${NSTACK_HOME:-$HOME/.nstack}/projects/\${SLUG:-unknown}/learnings.jsonl"
if [ -f "$_LEARN_FILE" ]; then
  _LEARN_COUNT=$(wc -l < "$_LEARN_FILE" 2>/dev/null | tr -d ' ')
  echo "LEARNINGS: $_LEARN_COUNT entries loaded"
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    ${ctx.paths.binDir}/nstack-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
# Session timeline: record skill start (local-only, never sent anywhere)
${ctx.paths.binDir}/nstack-timeline-log '{"skill":"${ctx.skillName}","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
# Check if CLAUDE.md has routing rules
_HAS_ROUTING="no"
if [ -f CLAUDE.md ] && grep -q "## Skill routing" CLAUDE.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
_ROUTING_DECLINED=$(${ctx.paths.binDir}/nstack-config get routing_declined 2>/dev/null || echo "false")
...
function generateUpgradeCheck(ctx: TemplateContext): string {
  return `If \`PROACTIVE\` is \`"false"\`, do not proactively suggest nstack skills AND do not
...
If A: run \`${ctx.paths.binDir}/nstack-config set telemetry community\`
...
If B→A: run \`${ctx.paths.binDir}/nstack-config set telemetry anonymous\`
If B→B: run \`${ctx.paths.binDir}/nstack-config set telemetry off\`
...
If A: run \`${ctx.paths.binDir}/nstack-config set proactive true\`
If B: run \`${ctx.paths.binDir}/nstack-config set proactive false\`
...
If B: run \`${ctx.paths.binDir}/nstack-config set routing_declined true\`
...
function generateVendoringDeprecation(ctx: TemplateContext): string {
  return `If \`VENDORED_NSTACK\` is \`yes\`: This project has a vendored copy of nstack at
...
3. Run \`${ctx.paths.binDir}/nstack-team-init required\` (or \`optional\`)
...
\`\`\`bash
${ctx.paths.binDir}/nstack-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
\`\`\`
...
function generateContextRecovery(ctx: TemplateContext): string {
  const binDir = ctx.host === 'codex' ? '$NSTACK_BIN' : ctx.paths.binDir;
...
\`\`\`bash
eval "$(${binDir}/nstack-slug 2>/dev/null)"
