...
NSTACK_ROOT="$HOME/.factory/skills/nstack"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.factory/skills/nstack" ] && NSTACK_ROOT="$_ROOT/.factory/skills/nstack"
NSTACK_BIN="$NSTACK_ROOT/bin"
NSTACK_BROWSE="$NSTACK_ROOT/browse/dist"
NSTACK_DESIGN="$NSTACK_ROOT/design/dist"
_UPD=$($NSTACK_BIN/nstack-update-check 2>/dev/null || .factory/skills/nstack/bin/nstack-update-check 2>/dev/null || true)
...
_TEL=$($NSTACK_BIN/nstack-config get telemetry 2>/dev/null || true)
...
for _PF in $(find ~/.nstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "$NSTACK_BIN/nstack-telemetry-log" ]; then
      $NSTACK_BIN/nstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
...
If A: run `$NSTACK_BIN/nstack-config set telemetry community`
...
If B→A: run `$NSTACK_BIN/nstack-config set telemetry anonymous`
If B→B: run `$NSTACK_BIN/nstack-config set telemetry off`
...
if [ "$_TEL" != "off" ] && [ -x $NSTACK_ROOT/bin/nstack-telemetry-log ]; then
  $NSTACK_ROOT/bin/nstack-telemetry-log \
...
[ -f .nstack/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.nstack/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.nstack/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.nstack/no-test-bootstrap` and re-run." Continue without tests.
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.factory/skills/nstack/review/TODOS-format.md`).
...
- Create an issue assigned to that person (use the platform detected in Step 0):
  - **If GitHub:**
    ...
    gh issue create \
      ...
      --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** nstack /ship on <date>" \
...
- Review report sections (`## NSTACK REVIEW REPORT`)
...
```bash
_CROSS_PROJ=$($NSTACK_BIN/nstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  $NSTACK_BIN/nstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
...
If A: run `$NSTACK_BIN/nstack-config set cross_project_learnings true`
If B: run `$NSTACK_BIN/nstack-config set cross_project_learnings false`
...
1. Read `.factory/skills/nstack/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
2. **Read `.factory/skills/nstack/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
source <($NSTACK_BIN/nstack-diff-scope <base> 2>/dev/null) || true
...
```bash
$NSTACK_BIN/nstack-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `$NSTACK_ROOT/review/specialists/testing.md`
2. **Maintainability** — read `$NSTACK_ROOT/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `$NSTACK_ROOT/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `$NSTACK_ROOT/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `$NSTACK_ROOT/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `$NSTACK_ROOT/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `$NSTACK_ROOT/review/design-checklist.md`
...
```bash
$NSTACK_BIN/nstack-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `$NSTACK_ROOT/review/specialists/red-team.md`
...
```bash
$NSTACK_ROOT/bin/nstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
Read `.factory/skills/nstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
OLD_CFG=$($NSTACK_ROOT/bin/nstack-config get codex_reviews 2>/dev/null || true)
...
```bash
$NSTACK_ROOT/bin/nstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
Read `.factory/skills/nstack/review/TODOS-format.md` for the canonical format reference.

**1. Check if TODOS.md exists** in the repository root.

**If TODOS.md does not exist:** Use AskUserQuestion:
- Message: "NStack recommends maintaining a TODOS.md organized by skill/component, then priority (P0 at top through P4, then Completed at bottom). See TODOS-format.md for the full format. Would you like to create one?"
