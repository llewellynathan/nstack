...
const ALL_SKILLS = (() => {
  ...
  if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
    skills.push({ dir: '.', name: 'root nstack' });
  }
...
describe('gen-skill-docs', () => {
  ...
  test('generated SKILL.md contains operational self-improvement (replaced contributor mode)', () => {
    ...
    expect(content).not.toContain('nstack_contributor');
    expect(content).not.toContain('contributor-logs');
    expect(content).toContain('Operational Self-Improvement');
    expect(content).toContain('nstack-learnings-log');
    expect(content).toContain('nstack-learnings-search --limit 3');
  });
  ...
  test('preamble .pending-* glob is zsh-safe (uses find, not shell glob)', () => {
    for (const skill of ALL_SKILLS) {
      ...
      expect(content).toContain("find ~/.nstack/analytics -maxdepth 1 -name '.pending-*'");
    }
  });
...
describe('REVIEW_DASHBOARD resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`review dashboard appears in ${skill} generated file`, () => {
      ...
      expect(content).toContain('nstack-review');
...
describe('PLAN_FILE_REVIEW_REPORT resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`plan file review report appears in ${skill} generated file`, () => {
      const content = fs.readFileSync(path.join(ROOT, skill, 'SKILL.md'), 'utf-8');
      expect(content).toContain('NSTACK REVIEW REPORT');
    });
  }
...
describe('Plan status footer in preamble', () => {
  test('preamble contains plan status footer', () => {
    ...
    expect(content).toContain('NSTACK REVIEW REPORT');
    expect(content).toContain('nstack-review-read');
...
describe('CODEX_SECOND_OPINION resolver', () => {
  const content = fs.readFileSync(path.join(ROOT, 'office-hours', 'SKILL.md'), 'utf-8');
  const codexContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'nstack-office-hours', 'SKILL.md'), 'utf-8');
  ...
  test('Codex host variant does NOT contain the Phase 3.5 resolver output', () => {
    ...
    expect(codexContent).not.toContain('nstack-codex-oh-');
  });
});
...
describe('Codex filesystem boundary', () => {
  ...
  test('codex skill has rabbit-hole detection rule', () => {
    ...
    expect(content).toContain('nstack-update-check');
  ...
  test('autoplan boundary text avoids host-specific paths for cross-host compatibility', () => {
    ...
    // autoplan template uses generic 'skills/nstack' pattern instead of host-specific
    ...
    expect(boundarySection).toContain('skills/nstack');
...
describe('Codex generation (--host codex)', () => {
  ...
  const CODEX_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('nstack')) {
        skills.push({ dir: '.', codexName: 'nstack' });
      }
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      ...
      const codexName = entry.name.startsWith('nstack-') ? entry.name : `nstack-${entry.name}`;
  ...
  test('root nstack bundle has OpenAI metadata for Codex skill browsing', () => {
    ...
    expect(content).toContain('display_name: "nstack"');
    expect(content).toContain('Use $nstack to locate the bundled nstack skills.');
    expect(content).toContain('allow_implicit_invocation: true');
  });

  test('externalSkillName mapping: root is nstack, others are nstack-{dir}', () => {
    // Root → nstack
    expect(fs.existsSync(path.join(AGENTS_DIR, 'nstack', 'SKILL.md'))).toBe(true);
    // Subdirectories → nstack-{dir}
    expect(fs.existsSync(path.join(AGENTS_DIR, 'nstack-review', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'nstack-ship', 'SKILL.md'))).toBe(true);
    // nstack-upgrade doesn't double-prefix
    expect(fs.existsSync(path.join(AGENTS_DIR, 'nstack-upgrade', 'SKILL.md'))).toBe(true);
    // No double-prefix: nstack-nstack-upgrade must NOT exist
    expect(fs.existsSync(path.join(AGENTS_DIR, 'nstack-nstack-upgrade', 'SKILL.md'))).toBe(false);
  });
  ...
  test('/codex skill excluded from Codex output', () => {
    expect(fs.existsSync(path.join(AGENTS_DIR, 'nstack-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'nstack-codex'))).toBe(false);
  });

  test('Codex review step stripped from Codex-host ship and review', () => {
    const shipContent = fs.readFileSync(path.join(AGENTS_DIR, 'nstack-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(AGENTS_DIR, 'nstack-review', 'SKILL.md'), 'utf-8');
  ...
  test('multiline descriptions preserved in Codex output', () => {
    ...
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'nstack-office-hours', 'SKILL.md'), 'utf-8');
    const fmEnd = content.indexOf('\n---', 4);
    const frontmatter = content.slice(4, fmEnd);
  ...
  test('hook skills have safety prose and no hooks: in frontmatter', () => {
    const HOOK_SKILLS = ['nstack-careful', 'nstack-freeze', 'nstack-guard'];
  ...
  test('Codex preamble resolves runtime assets from repo-local or global nstack roots', () => {
    // Check a skill that has a preamble (review is a good candidate)
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'nstack-review', 'SKILL.md'), 'utf-8');
  ...
  test('sidecar paths point to .agents/skills/nstack/review/ (not nstack-review/)', () => {
    // Regression: gen-skill-docs rewrote .claude/skills/review → .agents/skills/nstack-review
    // but setup puts sidecars under .agents/skills/nstack/review/. Must match setup layout.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'nstack-review', 'SKILL.md'), 'utf-8');
    // Correct: references to sidecar files use nstack/review/ path
    expect(content).toContain('.agents/skills/nstack/review/checklist.md');
    // design-checklist.md is now referenced via Review Army specialist (Claude only, stripped for Codex)
    // Wrong: must NOT reference nstack-review/checklist.md (file doesn't exist there)
    expect(content).not.toContain('.agents/skills/nstack-review/checklist.md');
  });

  test('sidecar paths in ship skill point to nstack/review/ for pre-landing review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'nstack-ship', 'SKILL.md'), 'utf-8');
    // Ship references the review checklist in its pre-landing review step
    if (content.includes('checklist.md')) {
      expect(content).toContain('.agents/skills/nstack/review/');
      expect(content).not.toContain('.agents/skills/nstack-review/checklist');
    }
  });
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toContain('.agents/skills/nstack/review/greptile-triage.md');
      expect(content).not.toContain('.agents/skills/nstack-review/greptile-triage');
    }
  });
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.claude/skills/nstack → $NSTACK_ROOT
    expect(content).not.toContain('~/.claude/skills/nstack');
    expect(content).toContain('$NSTACK_ROOT');

    // Rule 2: .claude/skills/nstack → .agents/skills/nstack
    expect(content).not.toContain('.claude/skills/nstack');

    // Rule 3: .claude/skills/review → .agents/skills/nstack/review
  ...
  test('path rewrite rules apply to all Codex skills with sidecar references', () => {
    ...
    for (const skill of CODEX_SKILLS) {
      ...
      if (content.includes('checklist.md') && !content.includes('design-checklist.md')) {
        expect(content).not.toContain('nstack-review/checklist.md');
      }
    }
  });
  ...
  test('Claude output unchanged: review skill still uses .claude/skills/ paths', () => {
    ...
    expect(content).toContain('~/.claude/skills/nstack');
  ...
  test('codex host produces empty outside voices in design-review', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'nstack-design-review', 'SKILL.md'), 'utf-8');
    expect(codexContent).not.toContain('Design Outside Voices');
  });

  test('codex host does not include Codex design block in ship', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'nstack-ship', 'SKILL.md'), 'utf-8');
...
describe('Factory generation (--host factory)', () => {
  ...
  const FACTORY_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('nstack')) skills.push({ dir: '.', factoryName: 'nstack' });
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      if (entry.name === 'codex') continue;
      if (!fs.existsSync(path.join(ROOT, entry.name, 'SKILL.md.tmpl'))) continue;
      const factoryName = entry.name.startsWith('nstack-') ? entry.name : `nstack-${entry.name}`;
  ...
  test('sensitive skills have disable-model-invocation', () => {
    const SENSITIVE = ['nstack-ship', 'nstack-land-and-deploy', 'nstack-guard', 'nstack-careful', 'nstack-freeze', 'nstack-unfreeze'];
  ...
  test('non-sensitive skills lack disable-model-invocation', () => {
    const NON_SENSITIVE = ['nstack-qa', 'nstack-review', 'nstack-investigate', 'nstack-browse'];
  ...
  test('/codex skill excluded from Factory output', () => {
    expect(fs.existsSync(path.join(FACTORY_DIR, 'nstack-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(FACTORY_DIR, 'nstack-codex'))).toBe(false);
  });

  test('Factory keeps Codex integration blocks', () => {
    // Factory users CAN use Codex second opinions (codex exec is a standalone binary)
    const shipContent = fs.readFileSync(path.join(FACTORY_DIR, 'nstack-ship', 'SKILL.md'), 'utf-8');
  ...
  test('Factory preamble uses .factory paths', () => {
    const content = fs.readFileSync(path.join(FACTORY_DIR, 'nstack-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('NSTACK_ROOT');
    expect(content).toContain('$_ROOT/.factory/skills/nstack');
    expect(content).toContain('$NSTACK_BIN/nstack-config');
  });
});
    ...
    describe(`${hostConfig.displayName} (--host ${hostConfig.name})`, () => {
      ...
      test('no .claude/skills path leakage in non-root skills', () => {
        ...
        for (const skill of skills) {
          // Skip root nstack skill — it contains preamble with intentional .claude/skills
          // fallback paths for binary lookup and skill prefix instructions
          if (skill === 'nstack') continue;
      ...
      if (hostConfig.generation.skipSkills?.includes('codex')) {
        test('/codex skill excluded', () => {
          expect(fs.existsSync(path.join(hostDir, 'nstack-codex', 'SKILL.md'))).toBe(false);
        });
      }
    });
  }
...
describe('setup script validation', () => {
  ...
  test('Codex install uses link_codex_skill_dirs', () => {
    ...
    expect(codexSection).not.toContain('ln -snf "$NSTACK_DIR" "$CODEX_NSTACK"');
  });
  ...
  test('setup separates install path from source path for symlinked repo-local installs', () => {
    expect(setupContent).toContain('INSTALL_NSTACK_DIR=');
    expect(setupContent).toContain('SOURCE_NSTACK_DIR=');
    expect(setupContent).toContain('INSTALL_SKILLS_DIR=');
    expect(setupContent).toContain('CODEX_NSTACK="$INSTALL_NSTACK_DIR"');
    expect(setupContent).toContain('link_codex_skill_dirs "$SOURCE_NSTACK_DIR" "$CODEX_SKILLS"');
  });
  ...
  test('Codex installs always create sidecar runtime assets for the real skill target', () => {
    ...
    expect(setupContent).toContain('create_agents_sidecar "$SOURCE_NSTACK_DIR"');
  });
  ...
  test('link_codex_skill_dirs reads from .agents/skills/', () => {
    ...
    expect(fnBody).toContain('nstack*');
  });
  ...
  test('link_claude_skill_dirs creates real directories with absolute SKILL.md symlinks', () => {
    ...
    // to ensure Claude Code discovers them as top-level skills (not nested under nstack/)
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('mkdir -p "$target"');
    expect(fnBody).toContain('ln -snf "$nstack_dir/$dir_name/SKILL.md" "$target/SKILL.md"');
  });
  ...
  // T1: Sidecar skip guard — prevents .agents/skills/nstack from being linked as a skill
  test('link_codex_skill_dirs skips the nstack sidecar directory', () => {
    const fnStart = setupContent.indexOf('link_codex_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('done', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('[ "$skill_name" = "nstack" ] && continue');
  });

  // T2: Dynamic $NSTACK_ROOT paths in generated Codex preambles
  test('generated Codex preambles use dynamic NSTACK_ROOT paths', () => {
    const codexSkillDir = path.join(ROOT, '.agents', 'skills', 'nstack-ship');
    ...
    expect(content).toContain('NSTACK_ROOT=');
    expect(content).toContain('$NSTACK_BIN/');
  });
  ...
  test('setup supports --host kiro with install section and sed rewrites', () => {
    ...
    expect(setupContent).toContain('~/.kiro/skills/nstack');
  });
  ...
  test('create_codex_runtime_root exposes only runtime assets', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('nstack/SKILL.md');
    expect(fnBody).toContain('browse/dist');
    expect(fnBody).toContain('browse/bin');
    expect(fnBody).toContain('nstack-upgrade/SKILL.md');
    ...
    expect(fnBody).not.toContain('ln -snf "$nstack_dir" "$codex_nstack"');
  });

  test('direct Codex installs are migrated out of ~/.codex/skills/nstack', () => {
    expect(setupContent).toContain('migrate_direct_codex_install');
    expect(setupContent).toContain('$HOME/.nstack/repos/nstack');
  ...
  test('link_claude_skill_dirs preserves already-prefixed dirs', () => {
    ...
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    // nstack-* dirs should keep their name (e.g., nstack-upgrade stays nstack-upgrade)
  ...
  test('cleanup_old_claude_symlinks removes only nstack-pointing symlinks', () => {
    ...
    expect(fnBody).toContain('nstack-*) continue');
  });
...
describe('discover-skills hidden directory filtering', () => {
  test('discoverTemplates skips dot-prefixed directories', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nstack-discover-'));
    try {
...
describe('community fixes wave', () => {
  ...
  // #594 — Discoverability: every SKILL.md.tmpl description contains "nstack"
  test('every SKILL.md.tmpl description contains "nstack"', () => {
    for (const skill of ALL_SKILLS) {
      const tmplPath = skill.dir === '.' ? path.join(ROOT, 'SKILL.md.tmpl') : path.join(ROOT, skill.dir, 'SKILL.md.tmpl');
      const content = fs.readFileSync(tmplPath, 'utf-8');
      const desc = extractDescription(content);
      expect(desc.toLowerCase()).toContain('nstack');
    }
  });
...
describe('codex commands must not use inline $(git rev-parse --show-toplevel) for cwd', () => {
  ...
  // Use Bun.Glob to avoid ELOOP from .claude/skills/nstack symlink back to ROOT
...
describe('LEARNINGS_SEARCH resolver', () => {
  ...
  for (const skill of SEARCH_SKILLS) {
    test(`${skill} generated SKILL.md contains learnings search`, () => {
      ...
      expect(content).toContain('nstack-learnings-search');
    });
  }
...
describe('gen-skill-docs prefix warning (#620/#578)', () => {
  ...
  test('warns about skill_prefix when config has prefix=true', () => {
    ...
    try {
      ...
      const fakeNstack = path.join(fakeHome, '.nstack');
      fs.mkdirSync(fakeNstack, { recursive: true });
      fs.writeFileSync(path.join(fakeNstack, 'config.yaml'), 'skill_prefix: true\n');
  ...
  test('no warning when skill_prefix is false or absent', () => {
    ...
    try {
      ...
      const fakeNstack = path.join(fakeHome, '.nstack');
      fs.mkdirSync(fakeNstack, { recursive: true });
      fs.writeFileSync(path.join(fakeNstack, 'config.yaml'), 'skill_prefix: false\n');
