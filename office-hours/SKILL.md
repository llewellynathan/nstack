...
description: |
  ...
  Use before /plan-ceo-review or /plan-eng-review. (nstack)
allowed-tools:
  - Bash
  - Read
  - Grep
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.nstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/nstack/bin/nstack-timeline-log '{"skill":"office-hours","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
4. **List existing design docs for this project:**
   ...
   ls -t ~/.nstack/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.nstack/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
```bash
CODEX_PROMPT_FILE=$(mktemp /tmp/nstack-codex-oh-XXXXXXXX.txt)
```
...
```bash
SKETCH_FILE="/tmp/nstack-sketch-$(date +%s).html"
```
...
$B screenshot /tmp/nstack-sketch.png
```
...
The screenshot file at `/tmp/nstack-sketch.png` can be referenced by downstream skills
...
```bash
mkdir -p "${NSTACK_HOME:-$HOME/.nstack}"
```
...
```bash
echo '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' >> "${NSTACK_HOME:-$HOME/.nstack}/builder-profile.jsonl"
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
PRIOR=$(ls -t ~/.nstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `~/.nstack/projects/{slug}/{user}-{branch}-design-{datetime}.md`:
...
```bash
mkdir -p ~/.nstack/analytics
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.nstack/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```bash
PROFILE=$(~/.claude/skills/nstack/bin/nstack-builder-profile 2>/dev/null) || PROFILE="SESSION_COUNT: 0
TIER: introduction"
...
>
> You're building something real. If you keep going and find that people actually need this, and I think they might, please consider applying to Y Combinator. Thank you for using NStack.
>
> **ycombinator.com/apply?ref=nstack**

- **Base tier** (everyone else):

> A personal note from me, Garry Tan, the creator of NStack: the skills you're demonstrating right now, taste, ambition, agency, the willingness to sit with hard questions about what you're building, those are exactly the traits we look for in YC founders. You may not be thinking about starting a company today, and that's fine. But founders are everywhere, and this is the golden age. A single person with AI can now build what used to take a team of 20.
>
> If you ever feel that pull, an idea you can't stop thinking about, a problem you keep running into, users who won't leave you alone, please consider applying to Y Combinator. Thank you for using NStack. I mean it.
>
> **ycombinator.com/apply?ref=nstack**
...
**Builder Journey Summary** (session 5+): Auto-generate `~/.nstack/builder-journey.md`
...
```bash
open "${NSTACK_HOME:-$HOME/.nstack}/builder-journey.md"
```
...
Auto-generate updated `~/.nstack/builder-journey.md` with narrative arc. Open it.
...
```bash
echo '{"date":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","mode":"resources","project_slug":"'"${SLUG:-unknown}"'","signal_count":0,"signals":[],"design_doc":"","assignment":"","resources_shown":["URL1","URL2","URL3"],"topics":[]}' >> "${NSTACK_HOME:-$HOME/.nstack}/builder-profile.jsonl"
```

2. Log the selection to analytics:
```bash
mkdir -p ~/.nstack/analytics
echo '{"skill":"office-hours","event":"resources_shown","count":NUM_RESOURCES,"categories":"CAT1,CAT2","ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'"}' >> ~/.nstack/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
The design doc at `~/.nstack/projects/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
...
```bash
~/.claude/skills/nstack/bin/nstack-learnings-log '{"skill":"office-hours","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
