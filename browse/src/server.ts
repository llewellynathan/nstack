/**
 * nstack browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.nstack/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.nstack/browse-{console,network,dialog}.log
...
function generateHelpText(): string {
  ...
  const lines = ['nstack browse — headless browser for AI agents', '', 'Commands:'];
...
const SESSIONS_DIR = path.join(process.env.HOME || '/tmp', '.nstack', 'sidebar-sessions');
...
function findBrowseBin(): string {
  const candidates = [
    ...
    path.resolve(__dirname, '..', '..', '.claude', 'skills', 'nstack', 'browse', 'dist', 'browse'),
    path.join(process.env.HOME || '', '.claude', 'skills', 'nstack', 'browse', 'dist', 'browse'),
  ];
...
function shortenPath(str: string): string {
  return str
    ...
    .replace(/\.claude\/skills\/nstack\//g, '')
...
function createWorktree(sessionId: string): string | null {
  try {
    ...
    const worktreeDir = path.join(process.env.HOME || '/tmp', '.nstack', 'worktrees', sessionId.slice(0, 8));
...
function spawnClaude(userMessage: string, extensionUrl?: string | null, forTabId?: number | null): void {
  ...
  const agentQueue = process.env.SIDEBAR_QUEUE_PATH || path.join(process.env.HOME || '/tmp', '.nstack', 'sidebar-agent-queue.jsonl');
  const nstackDir = path.dirname(agentQueue);
  ...
  try {
    fs.mkdirSync(nstackDir, { recursive: true, mode: 0o700 });
...
function killAgent(targetTabId?: number | null): void {
  ...
  const cancelDir = path.join(process.env.HOME || '/tmp', '.nstack');
...
async function shutdown() {
  ...
  const profileDir = path.join(process.env.HOME || '/tmp', '.nstack', 'chromium-profile');
...
async function start() {
  ...
  const server = Bun.serve({
    ...
    fetch: async (req) => {
      ...
      // Welcome page — served when NStack Browser launches in headed mode
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          // Check project-local designs first, then global
          const slug = process.env.NSTACK_SLUG || 'unknown';
          const homeDir = process.env.HOME || process.env.USERPROFILE || '/tmp';
          const projectWelcome = `${homeDir}/.nstack/projects/${slug}/designs/welcome-page-20260331/finalized.html`;
          if (fs.existsSync(projectWelcome)) return projectWelcome;
          // Fallback: built-in welcome page from nstack install
          const skillRoot = process.env.NSTACK_SKILL_ROOT || `${homeDir}/.claude/skills/nstack`;
        ...
        return new Response(
          `<!DOCTYPE html><html><head><title>NStack Browser</title>
          ...
          <body><div class="msg"><div class="gold">◈</div><p>NStack Browser ready.</p><p style="font-size:.85em">Waiting for commands from Claude Code.</p></div></body></html>`,
      ...
      if (url.pathname === '/tunnel/start' && req.method === 'POST') {
        ...
        try {
          // Read ngrok authtoken: env var > ~/.nstack/ngrok.env > ngrok native config
          let authtoken = process.env.NGROK_AUTHTOKEN;
          if (!authtoken) {
            const ngrokEnvPath = path.join(process.env.HOME || '', '.nstack', 'ngrok.env');
  ...
  // Reads NGROK_AUTHTOKEN from env or ~/.nstack/ngrok.env.
  ...
  if (process.env.BROWSE_TUNNEL === '1') {
    try {
      ...
      if (!authtoken) {
        console.error('[browse] BROWSE_TUNNEL=1 but no NGROK_AUTHTOKEN found. Set it via env var or ~/.nstack/ngrok.env');
      } else {
        const ngrok = await import('@ngrok/ngrok');
        const domain = process.env.NGROK_DOMAIN;
