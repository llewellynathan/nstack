...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/nstack-cli-');
    const execPath = path.join(root, '.claude/skills/nstack/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/nstack/browse/src/server.ts');
